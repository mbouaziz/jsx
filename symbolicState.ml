...
struct
  ...
  and svalue = ((svalue, unit) state, (svalue, srvalue) state list) _svalue
  and srvalue = (svalue, svalue sexn) rvalue

  type 'b t = (svalue, 'b) state
  type s = srvalue t
  type 'a _set = 'a t list
  type set = s list
  ...
  struct
    ...
    let state s =
      ...
      in
      ["assum", assumptions s s.pc; "pc", pathcondition s s.pc; "env", env s s.env; "heap", heap ~labs s s.heap; "res", res_rsvalue s s.res; "exn", res_exn s s.exn; "stk", callstack s s.callstack; "out", out s s.out]
      |> List.filter_map (fun (name, msg) -> if msg = "" then None else
			    Some (sprintf "%s:\t%s" name (String.interline "\t" msg)))
      |> String.concat "\n"

    let state_set = function
      | [] -> "NO STATE???"
  ...
  let res_v v s = [{ s with res = SValue v }]
  ...
  let res_id id s = res_v (Mk.sid id) s
  ...
  struct
    ...
    let branch f_t f_e v s =
      let sl_t = match PC.add (PredVal v) s.pc with
      | Some pc -> f_t { s with pc }
      | None -> []
      in
      let sl_e = match PC.add (PredNotVal v) s.pc with
      | Some pc -> f_e { s with pc }
      | None -> []
      in
      sl_t@sl_e
